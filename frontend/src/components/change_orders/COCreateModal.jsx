...
import { createChangeOrder } from '@/api/client'
...
export default function COCreateModal({ onClose, onCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.cost_impact || isNaN(parseFloat(form.cost_impact))) return setError('Cost impact must be a number.')
    setSaving(true)
    setError(null)
    try {
      const co = await createChangeOrder({
        ...form,
        cost_impact: parseFloat(form.cost_impact),
        schedule_impact: parseInt(form.schedule_impact) || 0,
        assigned_gc: form.assigned_gc || undefined,
      })
      onCreated(co)
      onClose()
    } catch {
      setError('Failed to create change order.')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-900">New Change Order</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1">Scope / Justification</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">Cost Impact (USD) *</label>
              <input type="number" min="0" step="1000" value={form.cost_impact} onChange={(e) => set('cost_impact', e.target.value)}
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="0" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">Schedule Impact (days)</label>
              <input type="number" min="0" value={form.schedule_impact} onChange={(e) => set('schedule_impact', e.target.value)}
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
